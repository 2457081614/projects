...
public class StaffView implements Serializable {
	...
	public void addBatch(FileUploadEvent event) {
		try {
			...
			// zy 密码加密
			List<Staff> staffsOld = Staff.getStaffs(sourceData);
			List<Staff> staffsNew = new ArrayList<Staff>();
			for (Staff s : staffsOld) {
				s.setPassword(Md5.makeMD5(s.getPassword()));
				staffsNew.add(s);
			}
			staffService.addStaffs(staffsNew);
	...
	public void setAllStaffList(List<Staff> allStaffList) {
	...
	public void addStaff(){
		...
		if(newStaff != null){
			...
			for(Staff staff:allStaffList){
				if(staff.getTel().equals(newStaff.getTel()) || staff.getEmail().equals(newStaff.getEmail()) 
						|| staff.getIdCardNumber().equals(newStaff.getIdCardNumber()))
					flag = false;
			}
			if(flag){
			...
			//zy
			s.setPassword(Md5.makeMD5(s.getPassword()));//加密密码
			...
			initNewStaff();
				...
				showResultTip("添加失败，此用户 手机/邮箱/身份证 信息已被注册", FacesMessage.SEVERITY_ERROR);
			}
		}else{
